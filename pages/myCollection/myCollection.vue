<template>
	<view class="index">
		<view class="collectionItem">
				<view class="itemTop">
					<view>共计<text style="color: #D43C43;padding: 0rpx 5rpx;">{{courseCollectionArr.length}}</text>件收藏</view>
				...
				<view class="itemMid">
					...
					<view class="billItem" v-for="(item,index) in courseCollectionArr" :key="index" @click="toDetail" :data-id='item.index'>
		...
		<view class="itemBottom">
			<view class="bottomBar" :hidden="!isEdit">
				...
				<button type="warn" class="cancelBtn" @click="onCancelCollection">取消收藏</button>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			toDetail(e){
				uni.navigateTo({
					url:`../secondHandDetail/secondHandDetail?id=${e.currentTarget.dataset.id}`
				})
			},
			// 编辑
			onEdit(){
